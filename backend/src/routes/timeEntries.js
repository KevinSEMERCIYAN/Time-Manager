...
router.post("/time-entries", authenticateJWT, async (req, res) => {
  try {
    ...
    // Règle métier: un seul pointage (IN/OUT) par jour et par utilisateur.
    // On calcule la date locale (Europe/Paris supposée) à partir de startTime.
    const localStart = new Date(start);
    const dayYMD = localStart.toISOString().slice(0, 10);
    ...
    if (existing) {
      // S'il existe déjà un pointage avec start+end pour ce jour, on bloque toute nouvelle création.
      if (existing.endTime) {
        return res.status(400).json({ error: "Un pointage existe déjà pour cette journée" });
      }
      // S'il existe un start sans end, on met simplement à jour la fin si l'appel fournit endTime.
      if (!end) {
        return res.status(400).json({ error: "Un pointage est déjà en cours pour cette journée" });
      }

      const endSql = `UPDATE time_entries SET end_time = ?, comment = COALESCE(?, comment) WHERE id = ?`;
      await db.query(endSql, [
